...
from multiprocessing import RawValue, Condition, RawArray
from typing import overload, Literal, Self

from libc.math cimport INFINITY, NAN, copysign, isnan

from . import LOGGER, PROFILE

cdef class _Contexts:
    cdef public int TICKER_SIZE
    cdef public int ID_SIZE
    cdef public int BOOK_SIZE

    def __init__(self, int TICKER_SIZE=16, int ID_SIZE=16, int BOOK_SIZE=10):
        self.TICKER_SIZE = TICKER_SIZE
        self.ID_SIZE = ID_SIZE
        self.BOOK_SIZE = BOOK_SIZE

Contexts = _Contexts(16, 16, 10)
...
class MarketData(object, metaclass=abc.ABCMeta):
    ...
    def from_bytes(cls, data: bytes) -> Self:
        ...
        if dtype_int == 10:
            buffer = _OrderBookBuffer.from_buffer_copy(data)
            return OrderBook.from_buffer(buffer=buffer)
        elif dtype_int == 20:
            buffer = _CandlestickBuffer.from_buffer_copy(data)
            return BarData.from_buffer(buffer=buffer)
        elif dtype_int == 21:
            buffer = _CandlestickBuffer.from_buffer_copy(data)
            return DailyBar.from_buffer(buffer=buffer)
        elif dtype_int == 30:
            buffer = _TickDataBuffer.from_buffer_copy(data)
            return TickData.from_buffer(buffer=buffer)
        elif dtype_int == 40:
            buffer = _TransactionDataBuffer.from_buffer_copy(data)
            return TransactionData.from_buffer(buffer=buffer)
        elif dtype_int == 41:
            buffer = _TransactionDataBuffer.from_buffer_copy(data)
            return TradeData.from_buffer(buffer=buffer)
        elif dtype_int == 50:
            buffer = _OrderDataBuffer.from_buffer_copy(data)
            return OrderData.from_buffer(buffer=buffer)
        else:
            raise ValueError(f'Invalid buffer type {dtype_int}\!')
    ...
    @property
    def topic(self) -> str:
        return str(f'{self.ticker}.{self.__class__.__name__}')
...
class OrderBook(MarketData):
    ...
    class Book(object):
        ...
        def __repr__(self):
            ...
            """
            return str(f'<OrderBook.Book.{"Bid" if self.side > 0 else "Ask"}>')
    ...
    def __init__(self, *, ticker: str, timestamp: float, bid: list[list[float | int]] = None, ask: list[list[float | int]] = None, **kwargs):
        ...
        buffer_constructor = _OrderBookBuffer
    ...
    def __repr__(self):
        ...
        """
        return str(f'<OrderBook>([{self.market_time:%Y-%m-%d %H:%M:%S}] {self.ticker}, bid={self.best_bid_price}, ask={self.best_ask_price})')
    ...
    def __str__(self):
        ...
        """
        return str(f'<OrderBook>([{self.market_time:%Y-%m-%d %H:%M:%S}] {self.ticker} {{Bid: {self.best_bid_price, self.best_bid_volume}, Ask: {self.best_ask_price, self.best_ask_volume}, Level: {self.max_level}}})')
...
class BarData(MarketData):
    ...
    def __init__(
        ...
        buffer_constructor = _CandlestickBuffer
    ...
    def __repr__(self):
        ...
        """
        return str(f'<{self.__class__.__name__}>([{self.market_time:%Y-%m-%d %H:%M:%S}] {self.ticker}, open={self.open_price}, close={self.close_price}, high={self.high_price}, low={self.low_price})')
...
class DailyBar(BarData):
    ...
    def __repr__(self) -> str:
        ...
        """
        return str(f'<{self.__class__.__name__}>([{self.market_time:%Y-%m-%d}] {self.ticker}, open={self.open_price}, close={self.close_price}, high={self.high_price}, low={self.low_price})')
...
class TickData(MarketData):
    ...
    def __init__(
        ...
        buffer_constructor = _TickDataBuffer

        buffer = buffer_constructor(
            ticker=ticker.encode('utf-8'),
            timestamp=timestamp,
            last_price=last_price,
            bid_price=NAN if bid_price is None else bid_price,
            bid_volume=NAN if bid_volume is None else bid_volume,
            ask_price=NAN if ask_price is None else ask_price,
            ask_volume=NAN if ask_volume is None else ask_volume,
    ...
    def __repr__(self) -> str:
        ...
        """
        return str(f'<TickData>([{self.market_time:%Y-%m-%d %H:%M:%S}] {self.ticker}, bid={self.bid_price}, ask={self.ask_price})')
...
class TransactionData(MarketData):
    ...
    def __init__(
        ...
        buffer_constructor = _TransactionDataBuffer
        ...
        buffer = buffer_constructor(
            ...
            multiplier=NAN if multiplier is None else multiplier,
            notional=NAN if notional is None else notional
        )
    ...
    def __repr__(self) -> str:
        ...
        """
        return str(f'<TransactionData>([{self.market_time:%Y-%m-%d %H:%M:%S}] {self.side.side_name} {self.ticker}, price={self.price}, volume={self.volume})')
    ...
    def merge(cls, trade_data_list: list[Self]) -> Self | None:
        ...
        trade_side_sign = 1 if sum_volume >= 0 else -1
        ...
        if sum_notional == 0:
            ...
            trade_price = sum_notional / sum_volume if sum_volume else copysign(math.inf, sum_notional)
...
class OrderData(MarketData):
    def __init__(
        ...
        buffer_constructor = _OrderDataBuffer
        id_size = buffer_constructor.id_size
    ...
    def __repr__(self) -> str:
        return str(f'<OrderData>([{self.market_time:%Y-%m-%d %H:%M:%S}] {self.side.side_name} {self.ticker}, price={self.price}, volume={self.volume})')
...
class MarketDataBuffer(object):
    ctype_buffer = _MarketDataBuffer
